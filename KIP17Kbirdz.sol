...
contract KIP17Kbirdz is KIP17, KIP17Enumerable, KIP17Metadata, MinterRole {
  ...
  function tokenURI(uint256 tokenId) public view returns (string memory) {
    // 여기는 약간 헷갈리는데 tokenId가 존재하는 지 안하는 지 체크하는 기능인 듯
    ...
    return
      bytes(currentBaseURI).length > 0
        ? string(
          abi.encodePacked(currentBaseURI, String.uint2str(tokenId), ".json")
        )
        : "";
  }

  // 이거는 스마트 컨트랙 한번 실행시키면 한번만 실행되는 거라고 했었지
  // 맨 처음 mintIndexForSale을 1로 지정해두었는데 아직 이유는 잘 모르겠어
  constructor() public {
    //init explicitly.
    _mintIndexForSale = 1;
  }

  // withdraw가 출금인가? 입금인가? 아 출금이네 아 이거가 내꺼 지갑으로 돈 보내주는 거네
  //   그 위에꺼는 출금 할 때마다 조코딩한테 5%씩 보내주는 거고
  function withdraw() external onlyMinter {
    // This code transfers 5% of the withdraw to JoCoding as a donation.
    // =============================================================================
    // 이거 살리면 큰일남
    // 0x3e944Ca8B08a0a0D3245B05ABF01586B9142f52C.transfer(
  ...
  function publicMint(uint256 requestedCount) external payable {
    ...
    require(
      _lastCallBlockNumber[msg.sender].add(_antibotInterval) < block.number,
      "Bot is not allowed"
    );
